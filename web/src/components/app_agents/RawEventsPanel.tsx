...
export const RawEventsPanel: React.FC<{
  transcript?: any[];
  events?: any[];
  fullWidth?: boolean;
}> = ({ transcript, events, fullWidth = false }) => {
  const items =
    Array.isArray(events) && events.length > 0 ? events : transcript || [];
  return (
    <aside
      className={`${
        fullWidth ? '' : 'xl:col-span-1'
      } bg-gray-900/70 border border-gray-800 rounded-lg p-4 flex flex-col h-[520px]`}>
